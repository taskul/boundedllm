...
from boundedllm.errors import GuardError
...
async def demo():
    with tempfile.TemporaryDirectory(prefix="boundedllm-demo-") as directory:
        settings = Settings(
...
def main():
    ...
    try:
        ...
        # Running a command before "boundedllm migrate" is the common first-run
        ...
        if missing:
            print(
                "database schema is missing or out of date; run 'boundedllm migrate' first",
                file=sys.stderr,
            )
        else:
...
def _run():
    ...
    try:
        if args.command == "init-db":
            ...
            from boundedllm.adapters.sql import migrate

            migrate(args.revision)
            print(json.dumps({"migrated_to": args.revision}))
            return
        elif args.command == "stamp":
            from boundedllm.adapters.sql import stamp
            ...
            import boundedllm.support.schema  # noqa: F401  registers domain tables
            ...
            from boundedllm.telemetry import OpenTelemetryHTTPSink, export_pending
