...
whatever already exists, and ``boundedllm.adapters.sql`` supplies a working
...
class Signer(Protocol):
    ...
    ``boundedllm.audit.Audit`` implements this and carries no storage of its own.
    """
...
class Ledger(Protocol):
    ...
    Implementations must treat the field allowlist in ``boundedllm.audit`` as the
