...
RoadShield is a complete, synthetic car-insurance web application used to exercise the `agentguard` package. It has password authentication, hashed database sessions, CSRF and Origin checks, a customer dashboard, tenant-scoped user data, policies, claims, an insurance chat agent, and a repeatable attack simulator.
